...
from event_logging.honeypot_event import HoneyPotSMBEventContent
    ...
    def queryPathInformation(connId, smbServer, recvPacket, parameters, data, maxDataCount=0):
        ...
        if connData['ConnectedShares'].has_key(recvPacket['Tid']):
            ...
            try:
                ...
                smbServer.log.debug("queryPathInformation: %s" % e)
...
class SMBCommands:
        ...
        if transParameters['ParameterCount'] != transParameters['TotalParameterCount']:
            ...
            if transCommands.has_key(command):
                ...
                try:
                    ...
                    smbServer.log.debug('Transaction: (%r,%s)' % (command, e))
                    errorCode = STATUS_ACCESS_DENIED
                    # raise
                ...
                smbServer.log.debug("Unsupported Transact command %r" % command)
                respParameters = ''
                respData = ''
        ...
        if NTTransParameters['ParameterCount'] != NTTransParameters['TotalParameterCount']:
            ...
            if transCommands.has_key(command):
                ...
                try:
                    ...
                    smbServer.log.debug('NTTransaction: (0x%x,%s)' % (command, e))
                    errorCode = STATUS_ACCESS_DENIED
                    # raise
                ...
                # smbServer.log.debug("Unsupported NTTransact command 0x%x" % command)
                respParameters = ''
                respData = ''
                errorCode = STATUS_NOT_IMPLEMENTED
        ...
        if trans2Parameters['ParameterCount'] != trans2Parameters['TotalParameterCount']:
            ...
            if transCommands.has_key(command):
                ...
                try:
                    ...
                    smbServer.log.debug('Transaction2: (0x%x,%s)' % (command, e))
                    # import traceback
                    # traceback.print_exc()
                    raise
                ...
                smbServer.log.debug("Unsupported Transact/2 command 0x%x" % command)
                respParameters = ''
                respData = ''
                errorCode = STATUS_NOT_IMPLEMENTED
    ...
    def smbComClose(connId, smbServer, SMBCommand, recvPacket):
        ...
        if connData['OpenedFiles'].has_key(comClose['FID']):
            ...
            try:
                ...
                smbServer.log.debug("comClose %s" % e)
                errorCode = STATUS_ACCESS_DENIED
            else:
    ...
    def smbComWrite(connId, smbServer, SMBCommand, recvPacket):
        ...
        if connData['OpenedFiles'].has_key(comWriteParameters['Fid']):
            ...
            try:
                ...
                smbServer.log.debug('smbComWrite: %s' % e)
                errorCode = STATUS_ACCESS_DENIED
        else:
            errorCode = STATUS_INVALID_HANDLE
    ...
    def smbComFlush(connId, smbServer, SMBCommand, recvPacket):
        ...
        if connData['OpenedFiles'].has_key(comFlush['FID']):
            ...
            try:
                ...
                smbServer.log.debug("comFlush %s" % e)
                errorCode = STATUS_ACCESS_DENIED
        else:
            errorCode = STATUS_INVALID_HANDLE
        ...
        if connData['ConnectedShares'].has_key(recvPacket['Tid']):
            ...
            # user access
                ...
                try:
                    ...
                    smbServer.log.debug("smbComCreateDirectory: %s" % e)
                    errorCode = STATUS_ACCESS_DENIED
        else:
            errorCode = STATUS_SMB_BAD_TID
        ...
        if connData['ConnectedShares'].has_key(recvPacket['Tid']):
            ...
            # user access
                ...
                try:
                    ...
                    smbServer.log.debug("smbComRename: %s" % e)
                    errorCode = STATUS_ACCESS_DENIED
        else:
            errorCode = STATUS_SMB_BAD_TID
        ...
        if connData['ConnectedShares'].has_key(recvPacket['Tid']):
            ...
            # user access
                ...
                try:
                    ...
                    smbServer.log.debug("smbComDelete: %s" % e)
                    errorCode = STATUS_ACCESS_DENIED
        else:
            errorCode = STATUS_SMB_BAD_TID
        ...
        if connData['ConnectedShares'].has_key(recvPacket['Tid']):
            ...
            # user access
                ...
                try:
                    ...
                    smbServer.log.debug("smbComDeleteDirectory: %s" % e)
    ...
    def smbComWriteAndX(connId, smbServer, SMBCommand, recvPacket):
        ...
        if connData['OpenedFiles'].has_key(writeAndX['Fid']):
            ...
            try:
                ...
                smbServer.log.debug('smbComWriteAndx: %s' % e)
                errorCode = STATUS_ACCESS_DENIED
        else:
            errorCode = STATUS_INVALID_HANDLE
    ...
    def smbComRead(connId, smbServer, SMBCommand, recvPacket):
        ...
        if connData['OpenedFiles'].has_key(comReadParameters['Fid']):
            ...
            try:
                ...
                smbServer.log.debug('smbComRead: %s ' % e)
                errorCode = STATUS_ACCESS_DENIED
        else:
            errorCode = STATUS_INVALID_HANDLE
    ...
    def smbComReadAndX(connId, smbServer, SMBCommand, recvPacket):
        ...
        if connData['OpenedFiles'].has_key(readAndX['Fid']):
            ...
            try:
                ...
                smbServer.log.debug('smbComReadAndX: %s ' % e)
                errorCode = STATUS_ACCESS_DENIED
        else:
            errorCode = STATUS_INVALID_HANDLE
    ...
    def smbComQueryInformation2(connId, smbServer, SMBCommand, recvPacket):
        ...
        if connData['OpenedFiles'].has_key(queryInformation2['Fid']):
            ...
            try:
                ...
                smbServer.log.debug('smbComQueryInformation2 %s' % e)
                errorCode = STATUS_ACCESS_DENIED
    ...
    def smbComNtCreateAndX(connId, smbServer, SMBCommand, recvPacket):
                ...
                if mode & os.O_CREAT == os.O_CREAT:
                    if createOptions & smb.FILE_DIRECTORY_FILE == smb.FILE_DIRECTORY_FILE:
                        try:
                            ...
                            smbServer.log.debug("NTCreateAndX: %s,%s,%s" % (pathName, mode, e))
                            errorCode = STATUS_ACCESS_DENIED
        ...
        if errorCode == STATUS_SUCCESS:
            ...
            if errorCode == STATUS_SUCCESS:
                ...
                # smbServer.log.debug('Create file %s, mode:0x%x' % (pathName, mode))
    ...
    def smbComOpenAndX(connId, smbServer, SMBCommand, recvPacket):
        ...
        if errorCode == STATUS_SUCCESS:
            ...
            # Let's store the fid for the connection
            # smbServer.log.debug('Opening file %s' % pathName)
    ...
    def smbComTreeConnectAndX(connId, smbServer, SMBCommand, recvPacket):
        ...
        if share is not None:
            ...
            resp['Tid'] = tid
            # smbServer.log.debug("Connecting Share(%d:%s)" % (tid,path))
        else:
            smbServer.log.debug("TreeConnectAndX not found %s" % path)
            ...
            if struct.unpack('B', sessionSetupData['SecurityBlob'][0])[0] == ASN1_AID:
                ...
                if len(blob['MechTypes'][0]) > 0:
                    ...
                    if mechType != TypesMech['NTLMSSP - Microsoft NTLM Security Support Provider']:
                        ...
                        smbServer.log.debug("Unsupported MechType '%s'" % mechStr)
    ...
    def smbComNegotiate(connId, smbServer, SMBCommand, recvPacket):
        ...
        try:
            ...
            smbServer.log.debug('smbComNegotiate: %s' % e)
    ...
    def default(connId, smbServer, SMBCommand, recvPacket):
        ...
        smbServer.log.debug("Not implemented command: 0x%x" % recvPacket['Command'])
...
class SMB2Commands:
    ...
    def smb2Negotiate(connId, smbServer, recvPacket, isSMB1=False):
        ...
        smbServer.logging_client.report_event('smb_cmd', HoneyPotSMBEventContent("127.0.0.1", "Negotiate"))
        ...
        smbServer.setConnectionData(connId, connData)
        smbServer.log.debug("Session negotiation done")
    ...
    def smb2SessionSetup(connId, smbServer, recvPacket):
        ...
        smbServer.logging_client.report_event('smb_cmd', HoneyPotSMBEventContent("127.0.0.1", "Session Setup"))
        connData = smbServer.getConnectionData(connId, checkStatus=False)
        ...
        # os.setreuid(65534,65534)
        smbServer.setConnectionData(connId, connData)
        smbServer.log.debug("Session setup done...")
        return [respSMBCommand], None, errorCode

    @staticmethod
    def smb2TreeConnect(connId, smbServer, recvPacket):
        smbServer.log.debug("SMB2 Tree Connect...")
        smbServer.logging_client.report_event('smb_cmd', HoneyPotSMBEventContent("127.0.0.1", "Tree Connect"))
        connData = smbServer.getConnectionData(connId)
        ...
        if share is not None:
            ...
            smbServer.log.debug("SMB2_TREE_CONNECT not found %s" % path)
            errorCode = STATUS_OBJECT_PATH_NOT_FOUND
        ...
        smbServer.setConnectionData(connId, connData)
        smbServer.log.debug("SMB2 Tree connect done...")
        return None, [respPacket], errorCode

    @staticmethod
    def smb2Create(connId, smbServer, recvPacket):
        smbServer.log.debug("SMB2 Create...")
        smbServer.logging_client.report_event('smb_cmd', HoneyPotSMBEventContent("127.0.0.1", "Create"))
        connData = smbServer.getConnectionData(connId)
    ...
    def smb2Close(connId, smbServer, recvPacket):
        ...
        smbServer.logging_client.report_event('smb_cmd', HoneyPotSMBEventContent("127.0.0.1", "Close"))
        connData = smbServer.getConnectionData(connId)
        ...
        smbServer.setConnectionData(connId, connData)
        smbServer.log.debug("SMB2 close done...")
        return [respSMBCommand], None, errorCode

    @staticmethod
    def smb2QueryInfo(connId, smbServer, recvPacket):
        smbServer.log.debug("SMB2 Query Info ...")
        smbServer.logging_client.report_event('smb_cmd', HoneyPotSMBEventContent("127.0.0.1", "Query Info"))
        connData = smbServer.getConnectionData(connId)
        ...
        smbServer.setConnectionData(connId, connData)
        smbServer.log.debug("SMB2 Query info done...")
        return [respSMBCommand], None, errorCode

    @staticmethod
    def smb2SetInfo(connId, smbServer, recvPacket):
        smbServer.log.debug("SMB2 Set Info ...")
        smbServer.logging_client.report_event('smb_cmd', HoneyPotSMBEventContent("127.0.0.1", "Set Info"))
        connData = smbServer.getConnectionData(connId)
        ...
        if connData['ConnectedShares'].has_key(recvPacket['TreeID']):
            ...
            if connData['OpenedFiles'].has_key(fileID):
                ...
                if setInfo['InfoType'] == smb2.SMB2_0_INFO_FILE:
                    ...
                    if informationLevel == smb2.SMB2_FILE_DISPOSITION_INFO:
                        ...
                        try:
                            ...
                            smbServer.log.debug("smb2SetInfo: %s" % e)
                            errorCode = STATUS_ACCESS_DENIED
                    else:
                        smbServer.log.debug('Unknown level for set file info! 0x%x' % informationLevel)
                        # UNSUPPORTED
                        errorCode = STATUS_NOT_SUPPORTED
                ...
                #    setInfo = queryFsInformation('/', fileName, queryInfo['FileInfoClass'])
                    ...
                    smbServer.log.debug("setInfo not supported (%x)" % setInfo['InfoType'])
        ...
        smbServer.setConnectionData(connId, connData)
        smbServer.log.debug("SMB2 Set info done...")
        return [respSMBCommand], None, errorCode

    @staticmethod
    def smb2Write(connId, smbServer, recvPacket):
        smbServer.log.debug("SMB2 write...")
        smbServer.logging_client.report_event('smb_cmd', HoneyPotSMBEventContent("127.0.0.1", "Write"))
        connData = smbServer.getConnectionData(connId)
        ...
        smbServer.setConnectionData(connId, connData)
        smbServer.log.debug("SMB2 write done...")
        return [respSMBCommand], None, errorCode

    @staticmethod
    def smb2Read(connId, smbServer, recvPacket):
        smbServer.log.debug("SMB2 read ...")
        smbServer.logging_client.report_event('smb_cmd', HoneyPotSMBEventContent("127.0.0.1", "Read"))
        connData = smbServer.getConnectionData(connId)
        ...
        smbServer.setConnectionData(connId, connData)
        smbServer.log.debug("SMB2 read done...")
        return [respSMBCommand], None, errorCode

    @staticmethod
    def smb2Flush(connId, smbServer, recvPacket):
        smbServer.log.debug("SMB2 flush ...")
        smbServer.logging_client.report_event('smb_cmd', HoneyPotSMBEventContent("127.0.0.1", "Flush"))
        connData = smbServer.getConnectionData(connId)
        ...
        smbServer.setConnectionData(connId, connData)
        smbServer.log.debug("SMB2 flush done...")
        return [respSMBCommand], None, errorCode

    @staticmethod
    def smb2QueryDirectory(connId, smbServer, recvPacket):
        smbServer.log.debug("SMB2 Query Directory ...")
        smbServer.logging_client.report_event('smb_cmd', HoneyPotSMBEventContent("127.0.0.1", "Query Directory"))
        connData = smbServer.getConnectionData(connId)
        ...
        smbServer.setConnectionData(connId, connData)
        smbServer.log.debug("SMB2 query directory done...")
        return [respSMBCommand], None, errorCode
    ...
    def smb2TreeDisconnect(connId, smbServer, recvPacket):
        ...
        smbServer.logging_client.report_event('smb_cmd', HoneyPotSMBEventContent("127.0.0.1", "Tree disconnect"))
        connData = smbServer.getConnectionData(connId)
        ...
        smbServer.setConnectionData(connId, connData)
        smbServer.log.debug("SMB2 Tree disconnect done...")
        return [respSMBCommand], None, errorCode

    @staticmethod
    def smb2Logoff(connId, smbServer, recvPacket):
        smbServer.log.debug("SMB2 logoff...")
        smbServer.logging_client.report_event('smb_cmd', HoneyPotSMBEventContent("127.0.0.1", "Logoff"))
        connData = smbServer.getConnectionData(connId)
        ...
        smbServer.setConnectionData(connId, connData)
        smbServer.log.debug("SMB2 logoff done...")
        return [respSMBCommand], None, errorCode

    @staticmethod
    def smb2Ioctl(connId, smbServer, recvPacket):
        smbServer.logging_client.report_event('smb_cmd', HoneyPotSMBEventContent("127.0.0.1", "IOCtl"))
        connData = smbServer.getConnectionData(connId)
...
class SMBSERVERHandler(SocketServer.BaseRequestHandler):
    ...
    def handle(self):
        self.__SMB.log.debug("Incoming connection (%s,%d)" % (self.__ip, self.__port))
        ...
        while True:
            try:
                ...
                self.__SMB.log.debug("Handle: %s" % e)
                # import traceback
                # traceback.print_exc()
                break

    def finish(self):
        self.__SMB.log.debug("Closing down connection (%s,%d)" % (self.__ip, self.__port))
...
class SMBSERVER(SocketServer.ThreadingMixIn, SocketServer.TCPServer):
    ...
    def processRequest(self, connId, data):
        ...
        isSMB2 = False
        SMBCommand = None
        self.log.debug("Processing new package")
        try:
            packet = smb.NewSMBPacket(data=data)
            SMBCommand = smb.SMBCommand(packet['Data'][0])
            self.log.debug("Packet was smb1")
        except:
            self.log.debug("Packet wasn't smb1...")
            # Maybe a SMB2 packet?
            try:
                packet = smb2.SMB2Packet(data=data)
                isSMB2 = True
                self.log.debug("But it was smb2!")
                self.log.debug(data)
            except:
                self.log.debug("And it wasn't smb2 either.")
                import traceback
                traceback.print_exc()
        ...
        try:
            ...
            if isSMB2 is False:
                if packet['Command'] == smb.SMB.SMB_COM_TRANSACTION2:
                    self.log.debug("SMB_COM_TRANSACTION2")
                    ...
                    self.log.debug("SMB_COM_NT_TRANSACT")
                    respCommands, respPackets, errorCode = self.__smbCommands[packet['Command']](
                        connId,
                        self,
                        SMBCommand,
                    ...
                    self.log.debug("SMB_COM_TRANSACTION")
                    respCommands, respPackets, errorCode = self.__smbCommands[packet['Command']](
                        connId,
                        self,
                        SMBCommand,
            ...
            self.log.debug('processRequest (0x%x,%s)' % (packet['Command'], e))
            raise
    ...
    def processConfigFile(self, configFile=None):
        ...
        self.log.debug(self.__serverConfig.get('global', 'server_os'))
        ...
        if credentials_fname is not "":
            ...
            while line:
                ...
                self.log.debug(self.__credentials)
                line = cred.readline()
            cred.close()
