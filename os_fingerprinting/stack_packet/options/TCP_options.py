...
class TCPOptions:
    ...
    def parse_from_dict(self, fp):
        ...
        if "T_MIN" in fp and "T_MAX" in fp:
            self.T = int(
                int(str(fp["T_MIN"]), 16) + (random.randint(0, pow(2, 32)) % (int(str(fp["T_MAX"]), 16) - int(str(fp["T_MIN"]), 16))))
        if "TG" in fp:
            self.TG = fp["TG"]
        if "W" in fp:
            self.W = int(fp["W"], 16)
        ...
        if "TS" in fp:
            ...
            if fp["TS"] == "U":
                ...
                self.TS_COUNT = 2000  # Most common result
        if "tsval" in fp:
    ...
    def __str__(self):
        return "TCP(" + "R=" + str(self.R) + " DF=" + str(self.DF) + " T_MIN=" + str(self.T_MIN) + " T=" + str(self.T) \
               + " T_MAX=" + str(self.T_MAX) + " TG=" + str(self.TG) + " W=" + str(self.W) + " S=" + str(self.S) \
               + " A=" + str(self.A) + " F=" + str(self.F) + " O=" + str(self.O) + " RD=" + str(self.RD) + " Q=" \
               + str(self.Q) + ")"
